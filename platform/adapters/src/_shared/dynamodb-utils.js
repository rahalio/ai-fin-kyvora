 ...
 */
export function getCoreTableName() {
    const tableName = process.env.TABLE_NAME || process.env.DYNAMODB_CORE_TABLE_NAME;
    if (!tableName) {
        throw new Error("TABLE_NAME or DYNAMODB_CORE_TABLE_NAME environment variable is not set. Please set one of these to your DynamoDB core table name.");
    }
    return tableName;
}
/**
 ...
 */
export function getBaseTableName() {
    const tableName = process.env.BASE_TABLE_NAME || process.env.DYNAMODB_BASE_TABLE_NAME;
    if (!tableName) {
        throw new Error("BASE_TABLE_NAME or DYNAMODB_BASE_TABLE_NAME environment variable is not set. Please set one of these to your DynamoDB base table name.");
    }
    return tableName;
}
/**
...
export function getAnalyticsTableName() {
    ...
    if (!tableName) {
        throw new Error("ANALYTICS_TABLE_NAME or DYNAMODB_ANALYTICS_TABLE_NAME environment variable is not set. Please set one of these to your DynamoDB analytics table name.");
    }
    return tableName;
}
/**
 ...
 */
export function getRealtimeTableName() {
    const tableName = process.env.REALTIME_TABLE_NAME || process.env.DYNAMODB_REALTIME_TABLE_NAME;
    if (!tableName) {
        throw new Error("REALTIME_TABLE_NAME or DYNAMODB_REALTIME_TABLE_NAME environment variable is not set. Please set one of these to your DynamoDB realtime table name.");
    }
    return tableName;
}
